...
export let characters = {
    ...
    "Мошенник": { energy: 10, stars: 0, isHavingPeoples: false, image: images["Мошенник"], needStars: 3, multiplier: 1, fullEnergy: 10},
    "Суперкоп": { energy: 10, stars: 0, isHavingPeoples: false, image: images["Суперкоп"], needStars: 3, multiplier: 1, fullEnergy: 10 }
};
//localStorage.setItem('characters', JSON.stringify(characters));  // - для обнуления всех параметров разкоменьтить
...
function drawCharacterInterface(character) {
    ...
    ctx.clearRect(0, 0, canvas.width, canvas.height); 
    MyText(`Энергия: ${character.energy} / ${character.fullEnergy}`, 0.016*canvasHeight, 0.04*canvasHeight, '#ffffff', font, 'Montserrat',);
    ...
    const buttons = [
        { x: 0.1*canvasWidth, y: 0.18*canvasHeight, width: 0.8*canvasWidth, height: 0.10*canvasHeight, text: `${selectedCharacter === 'Мошенник' ? 'Апгрейд' : 'Апгрейд'} ${formatNum(character.needStars)}`, isDisabled: (character.stars < character.needStars) },
...
const ActionHandler = (selectedCharacter, actionType) => {
    ...
    switch (actionType) {
        case ACTIONS['Способ']:
            ...
            character.energy = character.fullEnergy;
    ...
    switch (true) {
        ...
        case selectedCharacter === 'Article':
            if (y >= 0.88 * canvasHeight && y <= 0.96 * canvasHeight) {
                ...
                } else if (x >= 0.44 * canvasWidth && x <= 0.56 * canvasWidth && !isDisabled$$$) {
        ...
        case selectedCharacter === 'Question' && x >= 0.13*canvasWidth && x <= 0.87*canvasWidth :
            if(y >= 0.475 * canvasHeight && y <= 0.555*canvasHeight){
                ...
                selectedCharacter = 'Answer';
            }else if(x >= 0.5*canvasWidth && x <= 0.92*canvasWidth && y >= 0.88*canvasHeight && y <= 0.96*canvasHeight){
                selectedCharacter = 'Материалы';
                console.log('Материалы');
            }
            break;
        case selectedCharacter === 'Answer' && x >= 0.5*canvasWidth && x <= 0.92*canvasWidth && y >= 0.88*canvasHeight && y <= 0.96*canvasHeight:
            selectedCharacter = 'Материалы';
            console.log('Материалы');
            break;
        default:
...
export const draw = () => {
    if (!selectedCharacter) {
        ...
        drawArticle(selectedArticle, list, nameArticle[[selectedArticle-1]]);
...
export function updateEnergy(character){
    console.log(character + ': '+ characters[character].fullEnergy + ' --->');
    characters[character].fullEnergy = Math.round(characters[character].fullEnergy * 1.1);
    console.log('       --------->'+ characters[character].fullEnergy);
}
